 ...
 * @returns {*}
 */
export const runAsAdmin = (doc, filter) => {
 ...
 */
export const getUser = userId => Accounts.users.findOne(userId || FlowRouter.current().params.userId) ||
 ...
 * @returns {*}
 */
export const isUserLogs = () => FlowRouter.current().path.match(/dashboard\/users\/(.+)?/) ?
    getUser() : false;
...
export const is403 = (id, url) => {
  export const name = Meteor.isClient ? Template.instance().view.name : 'Meteor.server';
  throwError({
    error: 403,
